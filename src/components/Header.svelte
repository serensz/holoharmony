<script lang='ts'>
  import { isRandom, playlist, readInstructions, temporaryAttempt } from '$src/store';
  ...
  export const enableRandom = async () => {
    isRandom.set(true);
    if ($page.url.pathname.includes('/random')) {
      // don't endlessly add /random to the url
      // reset the temp. attempt because this is going to be a new custom game
